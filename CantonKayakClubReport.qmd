...
Data$Sunrise <- paste0(substr(Data$Sunrise, 1, 2), ":", substr(Data$Sunrise, 3, 4))
Data$Sunset <- paste0(substr(Data$Sunset, 1, 2), ":", substr(Data$Sunset, 3, 4))
...
Data$Dawn <- paste0(substr(Data$Dawn, 1, 2), ":", substr(Data$Dawn, 3, 4))
Data$Dusk <- paste0(substr(Data$Dusk, 1, 2), ":", substr(Data$Dusk, 3, 4))
...
#| include: FALSE
#| warning: FALSE
TheData <- merge(DawnData, SunData, by = c("Month", "Day"), all.x = TRUE)

TheData$Date <- make_date(year = 2024, month = match(TheData$Month, month.name), day = as.integer(TheData$Day))

Data <- TheData %>% arrange(Date) %>% relocate(Date, .before = Dawn) %>% select(-Month, -Day)
```
...
dst_start <- ymd("2024-03-10") 
dst_end <- ymd("2024-11-03") 
...
#PlotData <- DST_Data %>% filter(month(Date) %in% InterestedMonths)
PlotData <- DST_Data 
...
#ggsave(filename = "SunriseSunsets.png", AllSunrise, dpi = 600, width = 6, height = 4)
...
#| echo: FALSE
#| warning: FALSE
AllSunrise <- ggplotly(AllSunrise)
AllSunrise
```
...
TableProcessing <- function(x){
 ...
 TheFirstCol <- Luciernaga::NameCleanUp(TheFirstCol, TheStrings)
 TheFirstCol <- Luciernaga::NameCleanUp(TheFirstCol, " ")
 TheFirstCol <- data.frame(TheFirstCol)
